...
export default function CalendarPlanner({
  ...
  const handleConnectGoogleCalendar = async () => {
    ...
    try {
      ...
      if (e.response?.status === 503 || e.response?.data?.error?.includes('not configured')) {
        setGoogleSetupDialogOpen(true);
      } else {
        const msg = e.response?.data?.error || 'Failed to initiate Google Calendar connection.';
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      {/* ── GOOGLE CALENDAR SETUP & ALTERNATIVES DIALOG ────────────────────── */}
      <Dialog
        open={googleSetupDialogOpen}
        onClose={() => setGoogleSetupDialogOpen(false)}
        maxWidth="sm"
        fullWidth
        PaperProps={{ sx: { borderRadius: 3 } }}
      >
        <DialogTitle sx={{ bgcolor: '#F4F7FB', borderBottom: '1px solid #E0E7F1', pb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <Box>
              ...
              <Typography variant="caption" color="text.secondary">
                Auto-Push Configuration & 1-Click Sync Alternatives
              </Typography>
            </Box>
          </Box>
        </DialogTitle>
        ...
        <DialogContent dividers sx={{ p: 3 }}>
          <Alert severity="info" sx={{ mb: 2.5, borderRadius: 2 }}>
            <Typography variant="subtitle2" fontWeight={700}>
              Server OAuth Secret Pending
            </Typography>
            <Typography variant="body2" sx={{ fontSize: 13, mt: 0.5 }}>
              Automatic server-side push requires the administrator to configure <code>GOOGLE_CALENDAR_CLIENT_SECRET</code> in the backend environment.
            </Typography>
          </Alert>
          ...
          <Paper variant="outlined" sx={{ p: 2, mb: 1.5, borderRadius: 2, bgcolor: '#F8FAFC' }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ fontSize: 13, mb: 1.5 }}>
              Google Calendar will automatically keep your personal calendar in sync with all your planned visits. No server secrets needed!
            </Typography>
            <Button
              variant="contained"
              size="small"
              ...
              onClick={() => {
                ...
                setFeedback({
                  type: 'info',
                  text: 'Feed URL copied! In the new tab, paste into "URL of calendar" and click "Add calendar".',
                });
          ...
          <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: '#F8FAFC' }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ fontSize: 13 }}>
              On any visit card in your Month or Agenda view, click on the visit and select <strong>"Add to Google Calendar"</strong> to add that session directly to your personal Google schedule.
            </Typography>
          </Paper>
        </DialogContent>
