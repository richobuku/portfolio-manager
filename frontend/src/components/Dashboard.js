...
const NAV_ITEMS = [
  ...
  { key: 'tbip',           label: 'Improvement Plans', icon: <TrendingUp /> },
...
export default function Dashboard({ token, currentUser, onLogout }) {
  ...
  const [tbipFilterBge, setTbipFilterBge] = useState('all');
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [mRes, allMRes, eRes, cRes, gRes, sRes, tRes, aRes, uRes, rRes, grRes, pgRes, tbipRes] = await Promise.all([
        ...
        axios.get(API_ENDPOINTS.ENTERPRISE_IMPROVEMENT_PLANS, { headers: h }).catch(() => ({ data: [] })),
      ]);
      ...
      setTbipPlans(toArr(tbipRes.data));
      setError('');
    } catch {
  ...
  const COMM_TEMPLATES = [
    {
      key: 'bge_tbip_rollout',
      label: 'BGE — Enterprise Improvement Plan (TBIP) Rollout & Coaching Roadmap',
      subject: 'Action Required: Enterprise Improvement Plan (TBIP) Rollout & Monthly Coaching Roadmap',
      body: `Dear {{name}},

I hope this email finds you well and progressing smoothly with your field engagements.

As part of strengthening our Business Development Services (BDS) under PRUDEV II, we have launched a new tool on the BGE Portal: the Enterprise Improvement Plan (MSME Business Assessment & Technical Business Improvement Plan - TBIP).

---

1. PURPOSE & STRATEGIC IMPORTANCE
Conducting this diagnostic assessment and formulating the TBIP is now mandatory for each of your assigned MSMEs.

The TBIP serves as the strategic foundation and operational roadmap guiding your 3 monthly coaching visits per MSME. Every visit and intervention you carry out should directly track and address the priority gaps identified in this plan.

---

2. KEY STEPS IN THE ASSESSMENT PROCESS
When conducting the assessment with your MSMEs, please complete the following steps directly in the portal:

1. 7-Dimension Diagnostic Assessment: Complete the guided questionnaire covering Governance & Strategy, Financial Management, HR & Decent Work, Market & Customers, Digital & Technology, Environmental Sustainability, and Regulatory Compliance.
2. Gap Analysis & Observations: Review the auto-computed health status and record concise notes on the critical challenges identified.
3. Priority Action Roadmap (3–5 Actions): Agree on 3 to 5 high-impact, measurable actions with the MSME owner, specifying the responsible lead, expected timeline, and target outcomes.
4. BGE Sign-Off & Submission: Complete your BGE sign-off and submit the assessment through the portal.

---

3. REVIEW & APPROVAL MEETINGS
Once you submit the Enterprise Improvement Plans for your assigned portfolio:
- We will schedule an individual review meeting with you to walk through the identified diagnostic gaps, evaluate the proposed action plans, and grant final approval.
- You can also export and print the official branded PDF report or Excel workbook directly from the portal for your reference and to share with the enterprise owner.

---

4. NEXT STEPS & TIMELINES
Please log into the BGE Portal, navigate to the "Improvement Plans (TBIP)" tab (or launch directly from your MSME card), and begin conducting the assessments during your upcoming field visits.

Should you have any questions or require guidance on navigating the tool, please reach out directly.

Thank you for your continued dedication to supporting our MSMEs.
...
PRUDEV II BDS Team`
  ...
  const SMS_TEMPLATES = [
    {
      key: 'bge_tbip_reminder',
      label: 'BGE — Enterprise Improvement Plan (TBIP) Rollout',
      body: 'Dear {{name}}, please log in to the PRUDEV II portal to complete the new Enterprise Improvement Plan (TBIP) for your assigned MSMEs. This forms the basis of your 3 monthly visits. We will schedule a meeting to review and approve your submissions. — PRUDEV II BDS Team',
    },
    {
      key: 'data_reminder',
  ...
  const handleDownloadTbipExcel = async (planObj) => {
    try {
      ...
      setError('Failed to download Excel workbook');
    }
  };

  const renderTbip = () => {
    const filtered = tbipPlans.filter(p => {
      ...
      if (tbipFilterBge !== 'all' && String(p.bge) !== String(tbipFilterBge)) return false;
      if (tbipSearch.trim()) {
        const q = tbipSearch.toLowerCase();
        const mName = (p.msme_name || '').toLowerCase();
        const mCode = (p.msme_code || '').toLowerCase();
        const bName = (p.bge_name || '').toLowerCase();
        if (!mName.includes(q) && !mCode.includes(q) && !bName.includes(q)) return false;
      }
      return true;
    });

    const totalPlans = tbipPlans.length;
    const submittedCount = tbipPlans.filter(p => p.status === 'submitted').length;
    ...
    return (
      <Box>
        <SectionHeader
          title="Enterprise Improvement Plans (TBIP)"
          count={totalPlans}
          subtitle="MSME Business Diagnostic Assessments & Technical Improvement Roadmap"
          action={
            <Button
              variant="contained"
              startIcon={<Add />}
              onClick={() => {
                ...
                setSelectedTbipMsme(null);
                setTbipDialog(true);
              }}
              sx={{ bgcolor: BRAND.gizRed, '&:hover': { bgcolor: BRAND.gizDarkRed }, fontWeight: 700, borderRadius: 2 }}
            >
              New Assessment / Plan
            </Button>
          }
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={6} sm={3}>
            <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid #E2E8F0', bgcolor: '#FFFFFF' }}>
              <Typography variant="caption" color="text.secondary" fontWeight={600}>Total Assessments</Typography>
              <Typography variant="h5" fontWeight={800} color={BRAND.primaryMain}>{totalPlans}</Typography>
            </Paper>
          </Grid>
          <Grid item xs={6} sm={3}>
          ...
          <Grid item xs={6} sm={3}>
            <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid #93C5FD', bgcolor: '#EFF6FF' }}>
              <Typography variant="caption" color="primary.main" fontWeight={700}>Pending HOA Review</Typography>
              <Typography variant="h5" fontWeight={800} color="#1E40AF">{submittedCount}</Typography>
            </Paper>
          </Grid>
          <Grid item xs={6} sm={3}>
        ...
        <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid #CBD5E1', bgcolor: '#FFFFFF', mb: 2.5 }}>
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} sm={4}>
              <TextField
                size="small"
                placeholder="Search MSME name, code, or BGE..."
                value={tbipSearch}
            ...
            <Grid item xs={12} sm={2.5}>
              <FormControl size="small" fullWidth>
                <InputLabel>Lead BGE</InputLabel>
                <Select
                  label="Lead BGE"
                  value={tbipFilterBge}
                  onChange={(e) => setTbipFilterBge(e.target.value)}
                >
            ...
            <Table size="small" sx={{ minWidth: 760 }}>
              <TableHead sx={{ bgcolor: '#1A365D' }}>
                <TableRow>
                  ...
                  <TableCell sx={{ color: '#fff', fontWeight: 700 }}>Lead BGE</TableCell>
              ...
              <TableBody>
                {filtered.slice(tbipPage * ROWS_PER_PAGE, (tbipPage + 1) * ROWS_PER_PAGE).map((p) => {
                    ...
                    <TableRow key={p.id} hover>
                      ...
                      <TableCell align="right">
                        <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end', alignItems: 'center' }}>
                          <Tooltip title="Review / Approve / Edit Assessment">
                            <IconButton
                              size="small"
                              color="primary"
                              onClick={() => {
                                const mTarget = msmes.find(m => m.id === p.msme) || allMsmes.find(m => m.id === p.msme);
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      {/* ── Enterprise Improvement Plan (TBIP) Dialog (Admin / HOA view) ── */}
      <EnterpriseImprovementPlanDialog
        open={tbipDialog}
        onClose={() => setTbipDialog(false)}
        plan={selectedTbip}
        msme={selectedTbipMsme || (selectedTbip ? msmes.find(m => m.id === selectedTbip.msme) || allMsmes.find(m => m.id === selectedTbip.msme) : null)}
        bge={selectedTbip?.bge ? experts.find(e => e.id === selectedTbip.bge) : null}
        currentUser={currentUser}
        token={token}
        onSaved={(updated) => {
          fetchTbipPlans();
          setSelectedTbip(updated);
          setSuccess('Enterprise Improvement Plan updated.');
        }}
        notify={(msg, type) => {
