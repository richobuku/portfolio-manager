"""
Migration 0114 — Create Jacob Odur's Work Order for BGE Training on Bankable Documents & Feedback Session.

Creates PRUDEV II-CONS-JO-03 (5 days @ UGX 80,000/day, 28 Sep – 2 Oct 2026) in draft status.
...
def create_jacob_bankable_wo(apps, schema_editor):
...
def reverse_jacob_bankable_wo(apps, schema_editor):
    WorkOrder = apps.get_model('portfolio', 'WorkOrder')
    WorkOrder.objects.filter(work_order_number='PRUDEV II-CONS-JO-03').delete()


class Migration(migrations.Migration):

    dependencies = [
        ('portfolio', '0113_create_jacob_feedback_ai_work_order'),
    ]

    operations = [
        migrations.RunPython(create_jacob_bankable_wo, reverse_jacob_bankable_wo),
    ]
